...
def load_split(
    ...
    """
    Load a dataset split (train, test, or testF).

    Nota:
    - 'train'  -> entrenamiento
    - 'test'   -> validación durante entrenamiento
    - 'testF'  -> test final real (evaluación final)

    Parameters:
    -----------
    split_name : str
        Name of the split to load ('train', 'test', or 'testF')
    data_dir : str
    ...
    valid_splits = ['train', 'test', 'testF']
    if split_name not in valid_splits:
        raise ValueError(
            f"split_name must be one of {valid_splits}, got '{split_name}'"
        )
